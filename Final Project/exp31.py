...
class MyShortestForwarding(app_manager.RyuApp):
    ...
    def get_topology(self,ev):
        ...
        for link in link_list:
            self.network.add_edge(link.src.dpid, link.dst.dpid, weight=1, port=link.src.port_no)
    ...
    def get_out_port(self,datapath,src,dst,in_port):
        ...
        if src not in self.network:
            ...
            self.network.add_edge(dpid, src, weight=0,port=in_port)
            self.network.add_edge(src, dpid,weight=0)
