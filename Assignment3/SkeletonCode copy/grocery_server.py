...
sys.path.insert(0,'/home/xihanfu/Apps/flatbuffers')
...
from applnlayer.ApplnMessageTypes import code_type,contents_type,MessageTypes
...
class GroceryOrder ():
  ...
  def gen_response_msg (self):
    ...
    # how to find if I send bad request or ?
    # content set by yourself/ random set
    resp_msg.type = MessageTypes.RESPONSE
    resp_msg.code = code_type.Ok
    resp_msg.contents = contents_type.You_are_Healthy
  ...
  def driver (self):
    try:
      ...
      while True:
        ...
        if request.type == MessageTypes.GROCERY:
          resp = self.gen_response_msg()
          self.grocery_obj.send_response(resp)
        else:
          resp = self.gen_response_msg()
          resp.code = code_type.BAD_REQUEST
          resp.contents = contents_type.Bad_Request
          self.grocery_obj.send_response(resp)
...
def parseCmdLineArgs ():
  ...
  parser.add_argument ("-a", "--addr", default="127.0.0.1", help="Interface we are accepting connections on (default: all)")
